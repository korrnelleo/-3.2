...
function printMessages(thread, count) {
  ...
  const messagesToPrint = thread.messages.slice(-count);

  for (const message of messagesToPrint) {
    console.log(
      `${message.author.name} (репутация: ${message.author.reputation}): ${message.text}`,
    );
  }
}

printMessages(thread, 5);
